...
namespace franka_controllers
{
  ...
  void CartesianVelocityController::updateStatus()
  {
                                   ...
                                   const ros::Duration &dt)
  {
    ...
    double desired_v_y = 0.14 * e_v_y;                    // + 0.1 * de_v_y;      // kp = 0.1
    double desired_v_z = 0.10 * e_v_z;                    // + 0.65 * de_v_z;     // kp = 0.35, kd = 0.65
    double desired_w_x = 0.00035 * e_w_x + 0.64 * de_w_x; // kp = 0.0002, kd = 0.8
    double desired_w_y = 0.0017 * e_w_y + 0.2 * de_w_y;   // kp = 0.0008, kd = 0.2
  ...
  void CartesianVelocityController::starting(const ros::Time & /* time */)
  {
    ...
    isContact_msg =
        nh_.subscribe("isContact", 1, &CartesianVelocityController::isContact_callback, this);
    cmd_pos_msg = nh_.subscribe("franka_cmd_pos", 1, &CartesianVelocityController::cmd_pos_callback, this);
  ...
  void CartesianVelocityController::update(const ros::Time & /* time */, const ros::Duration &period)
  {
    ...
    for (size_t i = 0; i < 6; i++)
    {
      command[i] = 0.125 * command[i] + (1 - 0.125) * last_command[i]; // IIR filter
    }
